...
int main(){
	...
	gets( result0 );
	fflush( stdin );
	for (i = 0; i < np0; i++) tmdrec[i] = gs0[i];//所有原始公式转抄到tmdrec中
	np = i;//推理队列的尾部指针
	nPosLitter = 0;//文字的位置号
	nPosCond = 0;//条件的位置号
	isOk = 0;
	...
	while (true){
		...
		while ((i < np) && (tmdrec[i].isLitter != 1)) i++;
		if (i >= np) break;//找不到文字我就没法推理了
		...
		while (np > np0){ //从当前文字的下一个位置起寻找析取式，则一路往下走
			...
			for (i = 0; i < np; i++)//找到一个没有用过的戏曲式
				if ((tmdrec[i].isCond == 1) && (tmdrec[i].nUsed == 0)) break;
			if (i == np) break;//没有找到则结束推理，所有条件式都用到了
					...
					if (((strcmp( lastNiYou, tmdrec[i].niyou ) != 0) || ((strcmp( lastNiYou, tmdrec[i].niyou ) == 0) && tmdrec[i].niyou[0] != '('))){
						if ((tmdrec[i].gs[0] == '!') && (stmdp[0] != '!') && (strlen( tmdrec[i].gs ) - strlen( stmdp ) == 1)){    // !p+q p cuo
							...
							while (j < strlen( stmdp )){ //依次比较每个字符
								if (tmdrec[i].gs[j + 1] != stmdp[j]) break;//有一个不相等则结束比较
								j++;
							}
							...
							while (j < strlen( stmdp )){ //依次比较每一个字符
								if (tmdrec[i].gsLast[j + 1] != stmdp[j]) break;    //有一个不相等则结束比较
								j++;
							}
							...
							while (j < strlen( tmdrec[i].gsLast )){//依次比较每一个字符
								if (stmdp[j + 1] != tmdrec[i].gsLast[j]) break;    //有一个不相等则结束比较
								j++;
							}
		...
		if (isOk == 1) break;    //我推出来了，不要再找下一个文字了
	}
	if (isOk == 1) printf( "我推出来了,推理过程如下:\n" );
